...
File file = new File("tablemlp.csv");
Writer output = null;
...
for(int perc=10;perc<101;perc=perc+10)
{
	...
	for (int p=0;p<2;p++) 
	{
		...
		Classifier cModel = (Classifier)new MultilayerPerceptron(); //try for different classifiers and datasets
	...
	//eTest.crossValidateModel(cModel,mdata,10,mdata.getRandomNumberGenerator(1));
		...
		Classifier cModel2 = (Classifier)new MultilayerPerceptron();
		...
		Classifier cModel3 = (Classifier)new MultilayerPerceptron();
