...
public class randomeachcell {
	public static void main(String[] args) throws Exception{

	BufferedReader reader = new BufferedReader(
            new FileReader("spambase.arff"));
...
for(int perc=10;perc<101;perc=perc+10)
{
	...
	for (int p=0;p<2;p++) 
	{
		...
		/*for (int q=0;q<j;q++)
		{
			double sum1=0,sum2=0,avg1=0,avg2=0,n1=0,n2=0;
			for (int k=0;k<numBlock;k++)
			{
				if(trainwithmissing.instance(k).isMissing(q)==false)
				{
					n1++;
					sum1+=trainwithmissing.instance(k).value(q);
				}
				if(testwithmissing.instance(k).isMissing(q)==false)
				{
					n2++;
					sum2+=testwithmissing.instance(k).value(q);
				}
			}
			avg1=sum1/n1;avg2=sum2/n2;
			for (int k=0;k<numBlock;k++) 
			{	
				if(trainwithmissing.instance(k).isMissing(q)==true)
				{
					trainwithmissing.instance(k).setValue(q, avg1);
				}
				if(testwithmissing.instance(k).isMissing(q)==true)
				{
					testwithmissing.instance(k).setValue(q, avg2);
				}
			}
		}*/
	...
	//eTest.crossValidateModel(cModel,mdata,10,mdata.getRandomNumberGenerator(1));
		y11[p]=num0/numBlock*eTest1.areaUnderROC(0)+num1/numBlock*eTest1.areaUnderROC(1)/*+num2/numBlock*eTest1.areaUnderROC(2)*/;
