...
File file = new File("tableknn.csv");
Writer output = null;
...
for(int perc=10;perc<101;perc=perc+10)
{
	...
	for (int p=0;p<2;p++) 
	{
		...
		for (int q=0;q<j;q++)
		{
			...
			{	
				float toss = randomGenerator.nextFloat();
		...
		Classifier cModel = (Classifier)new IB1(); //try for different classifiers and datasets
	...
	//eTest.crossValidateModel(cModel,mdata,10,mdata.getRandomNumberGenerator(1));
		...
		Classifier cModel2 = (Classifier)new IB1();
		...
		Classifier cModel3 = (Classifier)new IB1();
