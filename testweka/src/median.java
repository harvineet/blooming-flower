...
for(int perc=10;perc<101;perc=perc+10)
{
	...
	for (int p=0;p<2;p++) 
	{
		...
		for (int q=0;q<j;q++)
		{
			...
			double[] A= new double[numBlock];
			double[] B= new double[numBlock];
			for (int k=0;k<numBlock;k++)
			{
				if(trainwithmissing.instance(k).isMissing(q)==false)
				{
					A[n1++]=trainwithmissing.instance(k).value(q);
				}
				if(testwithmissing.instance(k).isMissing(q)==false)
				{
					B[n2++]=testwithmissing.instance(k).value(q);
				}
			}
			med1= findMedian(A); med2=findMedian(B);
			for (int k=0;k<numBlock;k++) 
			{	
				if(trainwithmissing.instance(k).isMissing(q)==true)
				{
					trainwithmissing.instance(k).setValue(q, med1);
				}
				if(testwithmissing.instance(k).isMissing(q)==true)
				{
					testwithmissing.instance(k).setValue(q, med2);
				}
			}
		}
