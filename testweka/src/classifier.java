...
File file = new File("tablerf.csv");
Writer output = null;
...
for(int perc=10;perc<101;perc=perc+10)
{
	...
	for (int p=0;p<2;p++) 
	{
		...
		Classifier cModel = (Classifier)new RandomForest(); //try for different classifiers and datasets
	...
	//eTest.crossValidateModel(cModel,mdata,10,mdata.getRandomNumberGenerator(1));
		...
		Classifier cModel2 = (Classifier)new RandomForest();
		...
		Classifier cModel3 = (Classifier)new RandomForest();
