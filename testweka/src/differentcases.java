...
int numBlock= data.numInstances()*(data.numAttributes()-1)/2;
int numInst=i/2;
...
for(int perc=10;perc<81;perc=perc+10)
{
	...
	for (int p=0;p<2;p++) 
	{
		...
		for (int t=0;t<numInst;t++)
		  {
		  	...
		  	if (traindata.instance(t).classValue()==1.0) num1++;
		  	if (traindata.instance(t).classValue()==2.0) num2++;
		  }
		...
		while (k<numMissing) 
		{
			...
			if(reg[r][c]==0)
			{
				trainwithmissing.instance(r).setMissing(c);
				testwithmissing.instance(r).setMissing(c);
				reg[r][c]=1;
				k++;
			}
		}
		...
		y11[p]=num0/numInst*eTest1.areaUnderROC(0)+num1/numInst*eTest1.areaUnderROC(1)+num2/numInst*eTest1.areaUnderROC(2);
		y21[p]=eTest1.correct();
		y31[p]=num0/numInst*eTest1.fMeasure(0)+num1/numInst*eTest1.fMeasure(1)+num2/numInst*eTest1.fMeasure(2);
		...
		y12[p]=num0/numInst*eTest2.areaUnderROC(0)+num1/numInst*eTest2.areaUnderROC(1)+num2/numInst*eTest2.areaUnderROC(2);
		y22[p]=eTest2.correct();
		y32[p]=num0/numInst*eTest2.fMeasure(0)+num1/numInst*eTest2.fMeasure(1)+num2/numInst*eTest2.fMeasure(2);
		...
		y13[p]=num0/numInst*eTest3.areaUnderROC(0)+num1/numInst*eTest3.areaUnderROC(1)+num2/numInst*eTest3.areaUnderROC(2);
		y23[p]=eTest3.correct();
		y33[p]=num0/numInst*eTest3.fMeasure(0)+num1/numInst*eTest3.fMeasure(1)+num2/numInst*eTest3.fMeasure(2);
